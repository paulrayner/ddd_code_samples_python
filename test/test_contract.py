...
class TestContract(unittest.TestCase):
    ...
    def test_contract_is_set_up_correctly(self):
        ...
        self.assertEqual(contract.purchase_date, datetime.date(2010, 5, 7))
        self.assertEqual(contract.effective_date, datetime.date(2010, 5, 8))
        self.assertEqual(contract.expiration_date, datetime.date(2013, 5, 8))

    def test_contract_equality(self):
        product  = Product("dishwasher", "OEUOEU23", "Whirlpool", "7DP840CWDB0")
        contract1 = Contract(100.0, product, datetime.date(2010, 5, 8), datetime.date(2013, 5, 8), datetime.date(2010, 5, 7))
        contract2 = Contract(100.0, product, datetime.date(2010, 5, 8), datetime.date(2013, 5, 8), datetime.date(2010, 5, 7))
        contract3 = Contract(100.0, product, datetime.date(2010, 5, 8), datetime.date(2013, 5, 8), datetime.date(2010, 5, 7))
